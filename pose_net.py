...
class Pose_net():
    ...
    def draw_on_frame(frame, lines, drawing_color=(255,255,255), line_thickness=5):
        for line in lines:
            for i in range(1, len(line)):
                cv2.line(frame, line[i-1], line[i], drawing_color, line_thickness)
        return frame

    def draw_finger(idx, fnumber, frame, landmark):
        if idx == fnumber:  # Index finger tip landmark id
            h, w, c = frame.shape
            cx, cy = int(landmark.x * w), int(landmark.y * h)
            cv2.circle(frame, (cx, cy), 10, (255, 0, 255), cv2.FILLED)

    def draw(cap, hands, mp_hands,drawing_lines):
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    for idx, landmark in enumerate(hand_landmarks.landmark):
                        Pose_net.draw_finger(idx,8, frame, landmark)
                        Pose_net.draw_finger(idx,4, frame, landmark)
                    ...
                    if thumb_tip is not None and index_tip is not None:
                        ...
                        if distance < 50:  # Threshold distance
                            ...
                            if prev_point is not None:
                              Pose_net.drawing_lines[-1].append(index_tip)
                            else:
                                Pose_net.drawing_lines.append([index_tip])
                            prev_point = index_tip
            frame = Pose_net.draw_on_frame(frame, drawing_lines)
